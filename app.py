...
OBOBOBproduct_reviews = {}
OBOBOB
HTML_TEMPLATE = """
OBOBOB<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
OBOBOB    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
OBOBOB        .product-card:hover {
            transform: translateY(-5px);
            transition: all 0.3s ease;
        }
        .cart-badge, .wishlist-badge {
OBOBOB            position: absolute;
            top: -8px;
            right: -8px;
            background: #ef4444;
OBOBOB            color: white;
            border-radius: 50%;
            width: 20px;
            height: 20px;
            font-size: 12px;
...
OBOBOB                body: JSON.stringify({product_id: productId})
OBOBOB            })
            .then(response => response.json())
OBOBOBOBOBOB            .then(data => {
                cart = data.cart;
                updateCartUI();
            });
OBOBOBOBOBOB        }
...
OBOBOB            const cartItems = document.getElementById('cartItems');
...
OBOBOBOBOBOB                let subtotal = 0;
                cartItems.innerHTML = cart.map(item => {
OBOBOB                    subtotal += item.price * item.quantity;
OBOBOB                    return `
                        <div class="flex items-center border-b border-gray-200 py-4">
                            <img src="${item.image}" alt="${item.name}" class="w-16 h-16 object-cover rounded">
                            <div class="flex-1 ml-4">
...
OBOBOB
        function updateQuantity(productId, newQuantity) {
            if (newQuantity < 1) {
OBOBOB                removeFromCart(productId);
                return;
            }
...
OBOBOB            sidebar.classList.toggle('translate-x-full');
            overlay.classList.toggle('hidden');
        }
OBOBOB
...
OBOBOBOBOBOB            const searchTerm = e.target.value.toLowerCase();
OBOBOB            const products = document.querySelectorAll('.product-card');
OBOBOB            
            products.forEach(product => {
OBOBOB                const name = product.querySelector('h3').textContent.toLowerCase();
OBOBOB                const description = product.querySelector('p').textContent.toLowerCase();
OBOBOB                
...
OBOBOBOBOBOBOBOBOB        function scrollToProducts() {
...
OBOBOB            document.body.appendChild(notification);
OBOBOBOBOBOB            
OBOBOB            setTimeout(() => {
                notification.remove();
            }, 3000);
...
OBOBOB            .then(data => {
                if (data.success) {
OBOBOB                    showNotification('Order placed successfully!', 'success');
                    cart = [];
                    updateCartUI();
                    toggleCart();
                }
